...
from sklearn import linear_model, metrics, neural_network, svm
...
def ridge_model(train_input, train_target, test_input, weights_names):
    ...
    return preds, numpy.zeros(shape=(len(weights_container.keys())))#model.coef_
...
### freiburg, unknown, hamburg, leipzig
...
with open(os.path.join(out_f, 'main_text_{}_predictions_mlp.pkl'.format(pred_model)), 'wb') as o:
