...
with open(os.path.join('pkls', 'main_text_ridge_predictions_no-confound.pkl'), 'rb') as i:
...
out_f = os.path.join('prediction_plots_no-confound', 'main_text')
    ...
    for key in [
        ...
        for pov in legend_mapper.keys():
            ...
            for x, scats in zip(pov_xs, scatters):
                ax.scatter([x+random.sample([corr*0.01 for corr in range(-10, 10)], k=1)[0] for _ in range(min(iter_null_hyp, 1000))],
                            random.sample(scats.tolist(), k=min(iter_null_hyp, 1000)),
                           s=50,
                           color=colors[pov],
                           alpha=0.05,
                           )
...
for case in cases:
    for targ in xtimes:
        ...
        for i, x, y, scats, details in zip(range(len(targ_ys)), targ_xs, targ_ys, targ_scats, targ_details):
            ...
            ax.scatter([x+random.sample([corr*0.005 for corr in range(-10, 10)], k=1)[0] for _ in range(min(1000, iter_null_hyp))],
                        random.sample(scats.tolist(), k=min(1000, iter_null_hyp)),
                       s=50,
...
### correcting p values
hard_ps = dict()
for general_case in ['correlations',
                     #'weights',
                     ]:
