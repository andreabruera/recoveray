...
def visual_check(results_container, weights_container, iter_null_hyp, point_of_view, dict_t):
    ...
    print(['\n{} from {}'.format(dict_t, point_of_view), 'avg: {}'.format(round(t_avg, 3)), 'p: {}\n'.format(round(p, 3))])
    ### weights
...
with open(os.path.join('dataset', 'days_po_41.tsv')) as i:
    for l_i, l in enumerate(i):
        ...
        for h in header:
            if 'day' not in h:
                continue
            raw_data[h][l_i-21] = float(line[1])
assert l_i == 41
    ...
    for pr in predictors:
        ...
        for t in targets:
            ...
            for point_of_view in abilities:
                ...
                if 'T1' in confounds:
                    confounds_names = confounds_names+['T1_day_after_onset']
                elif 'T2' in confounds:
                    confounds_names = confounds_names+['T2_day_after_onset']
                confounds_names = list(set(confounds_names))
                print(confounds_names)
                ...
                del weights_container
                del confounds_names
    ### removing individual confounds among families of confounds
    for pr in predictors:
        if 'conn' in pr and '1' in pr:
            continue
        for t in targets:
            key, dict_t = read_dict_keys(name, t)
            if not ('3' in dict_t and '2' in dict_t):
                continue
            for point_of_view in abilities:
                if check_time_points(t, point_of_view, pr) == False:
                    continue
                ...
                # predictors from the other family become confounds too
                other_confounds_names = [k for l in predictors for k in labels[l] if int(l[-1])==int(point_of_view[-1]) and l==other_pr]
                all_confounds_names = orig_confounds_names+other_confounds_names
                for conf in all_confounds_names:
                    confounds_names = [c for c in all_confounds_names if c!= conf]
                    confounds_names = confounds_names+['{}_day_after_onset'.format(point_of_view)]+['{}_day_after_onset'.format(t)]
                    if 'T1' in confounds:
                        confounds_names = confounds_names+['T1_day_after_onset']
                    elif 'T2' in confounds:
                        confounds_names = confounds_names+['T2_day_after_onset']
                    ...
                    ### just printing
                    print('target: {} only {} w/o confound {}, from {}'.format(dict_t, pr, conf, point_of_view))
                    print('\n')
                    print('predictors: {}'.format(weights_names))
                    print('confounds: {}'.format(confounds_names))
                    ...
                    ### storing after iter_null_hyp*folds iterations
                    last_key = '{} no confound {}'.format(pr, conf)
                    collector[key][point_of_view][dict_t]['confounds'][last_key] = confounds_names
                    collector[key][point_of_view][dict_t]['correlations'][last_key] = results_container
                    ### just a visual check
                    visual_check(results_container, weights_container, iter_null_hyp, point_of_view, dict_t)
...
with open(os.path.join(out_f, 'main_text_{}_predictions_ablations_confounds.pkl'.format(pred_model)), 'wb') as o:
