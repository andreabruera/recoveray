...
with open(os.path.join('pkls_r2', 'si_ridge_predictions.pkl'), 'rb') as i:
...
out_f = os.path.join('prediction_plots_r2', 'SI')
    ...
    for key in [
        ...
        for pov in ['all']:
            ...
            for target in targets:
                try:
                    curr_corrs = all_res[general_case][pov][target]['r2'][curr_key]
                except KeyError:
                    continue
                ### p value
                #p_container = numpy.average(curr_corrs, axis=1)
                p_container = curr_corrs
            ...
            for x, scats in zip(pov_xs, scatters):
                ax.scatter([x+random.sample([corr*0.01 for corr in range(-10, 10)], k=1)[0] for _ in range(599)],
                            random.sample(scats.tolist(), k=599),
                           s=50,
                           color=colors[pov],
                           alpha=0.05,
                           )
### correcting p values
        ...
        pyplot.ylabel('R-squared', fontsize=20, fontweight='bold')
    ...
    for targ in xtimes:
        ...
        for pred_fam in pred_fams:
            for pov in ['all']:
                key = pred_fam
                try:
                    curr_corrs = all_res[case][pov][targ]['r2'][key]
                except KeyError:
                    continue
                #p_container = numpy.average(curr_corrs, axis=1)
                p_container = curr_corrs
        ...
        for i, x, y, scats, details in zip(range(len(targ_ys)), targ_xs, targ_ys, targ_scats, targ_details):
            ...
            ax.scatter([x+random.sample([corr*0.005 for corr in range(-10, 10)], k=1)[0] for _ in range(599)],
                        random.sample(scats.tolist(), k=599),
                       s=50,
...
ax.set_ylim(bottom=-.15, top=0.15)
ax.legend(
          loc=9,
          fontsize=18,
          ncols=6,
            ...
            for targ in all_res[case][pov].keys():
                for fam in pred_fams:
                    ...
                    if general_case == 'r2':
                        ...
                        basic_corrs = all_res[case][pov][targ][general_case]['{}'.format(fam)]
                    ...
                    for x_i, x in enumerate(x_names):
                        ...
                        if general_case == 'r2':
                            ...
                            for x_i_two, x_two in enumerate(x_names):
                                ...
                                other_container = all_res[case][pov][targ][general_case][x_two]-basic_corrs
            ...
            for targ in all_res[case][pov].keys():
                for fam in pred_fams:
                    ...
                    if general_case == 'r2':
                        ...
                        basic_corrs = all_res[case][pov][targ][general_case][fam]
                        basic_avg = numpy.nanmean(basic_corrs)
                    else:
