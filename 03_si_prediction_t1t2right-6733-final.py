    ...
    for pr in predictors:
        for t in targets:
            if pr not in ['age', 'lesions'] and int(pr[-1]) >= int(t[-1]):
                continue
            for point_of_view in ['all']:
                key, dict_t = read_dict_keys(name, t)
                ### updating collector dict
                collector = check_collector(collector, key, dict_t, point_of_view=point_of_view)
                ...
                if 'impro' in name:
                    ...
                    other_predictors = [l for l in predictors if (l in ['age', 'lesions'] and l!=pr)]
                    ### confounds
                    orig_confounds_names = [k for l in confounds for k in labels[l] if int(l[-1])==int(t[1]) or len(k)==2]
                else:
                    other_predictors = [l for l in predictors if l in ['age', 'lesions'] and l!=pr]

                    ### confounds
                    orig_confounds_names = [k for l in confounds for k in labels[l] if int(l[-1])==int(t[-1])]
                assert len(other_predictors) in [1, 2, 3]
...
with open(os.path.join(out_f, 'si_{}_predictions_t1t2right-6733-final.pkl'.format(pred_model)), 'wb') as o:
