...
with open(os.path.join('pkls', 'si_ridge_predictions_t1t2right-6733-final.pkl'), 'rb') as i:
...
out_f = os.path.join('prediction_plots_t1t2right-6733-final', 'SI')
...
### correcting p values
        ...
        ax.text(s='Language {}'.format(mapper[general_case]), x=1, y=.52, fontsize=25, fontweight='bold', fontstyle='italic', va='center', ha='center')
...
legend_mapper = {
        'age all' : 'Age',
        'lesions all': 'Lesion affection',
        'T1 all' : 'Acute language ability',
        'T2 all' : 'Subacute language ability',
        }
...
for case in cases:
    for targ in xtimes:
        ...
        if len(targ_ys) == 3:
            ...
            continue
        corrections = [corr for corr_i, corr in enumerate(numpy.linspace(-corrs, corrs, len(targ_ys)+len(missing_idxs))) if corr_i not in missing_idxs]

        targ_xs = [base_x+corr for corr_i, corr in enumerate(corrections)]
        print(targ_xs)
        print(targ_ys)
        ...
        for i, x, y, scats, details in zip(range(len(targ_ys)), targ_xs, targ_ys, targ_scats, targ_details):
            ...
            ax.bar(x, y, width=0.15, alpha=0.6,  color=pred_colours[curr_key])
            if curr_key not in label_set:
...
colours = {
        ...
        'lesions' : {
            'L_SMA' : 'thistle',
            'L_DLPFC' : 'mediumpurple',
            'L_IFGorb' : 'aquamarine',
            'L_PTL' : 'mediumaquamarine',
            },
    'connectivity' : {
