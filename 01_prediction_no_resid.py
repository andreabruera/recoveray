...
for name, targets in [
    ...
    for t in targets:
        ...
        for point_of_view in abilities:
            ...
            it_predictors = numpy.array([full_data[k] for k in weights_names]+[full_data[k] for k in confounds_names]).T
            ...
            for null in tqdm(range(iter_null_hyp)):
                ...
                for fold in range(n_folds):
                    ...
                    # weights
                    #print(coefs.shape)
                    assert coefs.shape == (len(weights_container.keys())+len(confounds_names),)
    ...
    for pr in predictors:
        for t in targets:
            ...
            for point_of_view in abilities:
                ...
                #it_predictors = numpy.array([full_data[k] for k in weights_names]+[full_data[k] for k in confounds_names]).T
                it_predictors = numpy.array([full_data[k] for k in confounds_names]).T
                it_confounds = numpy.array([full_data[k] for k in confounds_names]).T
                it_target = numpy.array(full_data[t])
                results_container = numpy.zeros(shape=(iter_null_hyp, n_folds))
                weights_container = {k : numpy.zeros(shape=(iter_null_hyp, n_folds)) for k in weights_names}
                for null in tqdm(range(iter_null_hyp)):
                    ...
                    for fold in range(n_folds):
                        ...
                        assert coefs.shape == (len(confounds_names),)
                        for weight_i, weight in enumerate(weights_names):
                            weights_container[weight][null, fold] = coefs[weight_i]
                        # true values
                        all_targs.append(test_target)
...
with open(os.path.join(out_f, 'main_text_{}_predictions_no-confound_all-together.pkl'.format(pred_model)), 'wb') as o:
