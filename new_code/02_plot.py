...
line_colors = {
          'traditional' : [168, 168, 168],
          #'functional' : [227, 190, 107],
          'functional' : [240, 228, 66],
         }
...
for act_type in [
    ...
    ### main plots
    with open(os.path.join('pkls', act_type, 'full_results.pkl'), 'rb') as o:
    ...
    for analysis_type in [
        ...
        threshold[analysis_type] = 1.
        if analysis_type == 'functional':
            out = os.path.join('plots', act_type, 'main_text', 'all_variables')
            os.makedirs(out, exist_ok=True)
        else:
            out = os.path.join('plots', act_type, 'SI', 'all_variables')
            os.makedirs(out, exist_ok=True)
            ...
            for mode in modalities:
                ...
                for targ_idx, targ in enumerate(case_targets[case]):
                    ...
                    for pov_idx, pov in enumerate(povs):
                        ...
                        real_sem=scipy.stats.sem(case_results[mode][targ_idx, pov_idx, 0, :])
                        ...
                        ### raw p
                        raw_p = ps[case][mode][targ_idx, pov_idx, 0]
                        if p<.8:
                            #print('raw: {}, fdr: {}'.format(raw_p, p))
                            assert raw_p < p
                        if p<0.05:
                            threshold[analysis_type]=min(real_avg, threshold[analysis_type])
                        if p<0.005:
                            ax.scatter(
    ...
    with open(os.path.join('pkls', act_type, 'var-by-var_results.pkl'), 'rb') as o:
        var_by_var_results = pickle.load(o)


    matrices = {
                'activity' : numpy.zeros(shape=(7, 20)),
                'connectivity' : numpy.zeros(shape=(7, 29)),
                'lesion' : numpy.zeros(shape=(7, 4)),
                }
    matrices_ps = {
                'activity' : numpy.zeros(shape=(7, 20)),
                'connectivity' : numpy.zeros(shape=(7, 29)),
    ...
    for analysis_type in [
                    ...
                    for pov_idx, pov in enumerate(povs):
                        ...
                        if p<0.05:
                            ...
                            for curr_pred_i, curr_pred in enumerate(curr_preds):
                                ...
                                #print('{} {} {} {} {} {} {} full: {} removed: {} percent: {}'.format(curr_pred, case, mode, targ, pov, avg_w, numpy.nanmean(perm_avg_w), full_avg, avg, decrease))
                                p = min(1, ((sum([1 for _ in perm_avg_w if abs(_)>abs(avg_w)])+1)/iterations)*2)
                                chosen.append((case, mode, targ, pov, curr_pred, decrease, avg_w, avg, p))
        ### 0-> raw p, 1->fdr p
        ...
        #print(fdr_ps)
        print('FDR threshold: {}'.format(threshold[analysis_type]))
        for case, case_results in results.items():
            mtrx_start = 0 if case =='ability' else 4
            for mode in modalities:
                if mode == 'activity':
                    shape = (11, 10)
                    p_y=10
                ...
                for targ_idx, targ in enumerate(case_targets[case]):
                    ...
                    for pov_idx, pov in enumerate(povs):
                        ...
                        if analysis_type == 'functional':
                            out = os.path.join('plots', act_type, 'main_text', 'var_by_var', mode, case)
                            os.makedirs(out, exist_ok=True)
                        else:
                            out = os.path.join('plots', act_type, 'SI', 'var_by_var', mode, case)
                            os.makedirs(out, exist_ok=True)
                        fig, ax = pyplot.subplots(
                                                  figsize=shape,
                                                  tight_layout=True,
                        ...
                        for counter, idx in enumerate(curr_chosen):
                            curr_case, curr_mode, curr_targ, curr_pov, curr_pred, decrease, avg_w, decrease_height, p = chosen[idx]
                            matrices[mode][mtrx_x, mtrx_y_start+counter] = decrease
                            matrices_ps[mode][mtrx_x, mtrx_y_start+counter] = decrease_height
                            fdr_p = fdr_ps[idx]
                            ...
                            all_xticks[mode] = [w[1].split('_')[-1].replace('2', '\nto\n').replace('left-', 'L ').replace('right-', 'R ') for w in xticks]
                            start += 1
                            ...
                            # we keep only cases where the value falls below the FDR threshold
                            if decrease_height < threshold[analysis_type]:
                                ax.scatter(
                        ...
                        pyplot.clf()
                        pyplot.close()
    ### plotting trial matrices
    out = os.path.join('plots', act_type, 'main_text', 'matrices')
    os.makedirs(out, exist_ok=True)
    for k, v in matrices.items():
        ...
        for mult in range(3):
            start = (end*mult)+mult
            curr_end = start+end
            curr_title = '{} {}'.format(xticks_mapper[case_targets['ability'][mult]], k)
            curr_v = v[start:curr_end, :]
            curr_ps = matrices_ps[k].T[start:curr_end, :]

            fig, ax = pyplot.subplots(figsize=figsize, tight_layout=True)
            plot_mtrx = ax.imshow(-curr_v,
                                  vmin=0,
                                vmax=100,
                                cmap='plasma',
                                )
            thr = threshold['functional'] if k!='lesion' else threshold['traditional']
            pyplot.xticks(ticks=range(7), labels=[xticks_mapper[c] for c in case_targets['ability']]+['']+[xticks_mapper[c] for c in case_targets['improvement']], fontweight='bold', fontsize=16)
            ax.hlines(y=[_-.5 for _ in range(curr_v.shape[0])], xmin=-.5, xmax=curr_v.shape[1]-.5, color='silver', linewidth=1, zorder=2.95)
            ax.vlines(x=[_-.5 for _ in range(curr_v.shape[1])], ymin=-.5, ymax=curr_v.shape[0]-.5, color='silver', linewidth=1, zorder=2.95)
            if k == 'activity':
                vlines = [5.5, 11.5]
                pyplot.yticks(ticks=range(curr_v.shape[0]), labels=all_xticks[k], fontsize=15, fontweight='bold')
            elif k == 'connectivity':
                if mult == 2:
                    continue
                vlines = [8.5, 17.5]
                ...
                vlines = []
                pyplot.yticks(ticks=range(curr_v.shape[0]), labels=all_xticks[k], fontsize=15, fontweight='bold')
            ### whiting out predictors
            for x in range(curr_v.shape[0]):
                for y in range(curr_v.shape[1]):
                    if curr_v[x, y] == 0:
                        ax.scatter(y, x, marker='s', color='white', s=sc)
                        #ax.text(y, x, s='n.s.', color='black', zorder=3, ha='center', va='center')
                        continue
                    ax.text(x=y, y=x, s=round(curr_ps[x, y], 4), color='white', fontsize=20, zorder=2.85, ha='center',va='center')
                    ### bounding box
                    if curr_ps[x, y] < thr and curr_ps[x, y]!=0:
                        ax.hlines(y=[x-.4, x+.4], xmin=y-.4, xmax=y+.4, color='white', linestyle='dashed', linewidth=7.5, zorder=2.85)
                        ax.vlines(x=[y-.4, y+.4], ymin=x-.4, ymax=x+.4, color='white', linestyle='dashed', linewidth=7.5, zorder=2.85)
            ax.set_title(curr_title, fontsize=40, fontweight='bold')
            pyplot.savefig(os.path.join(out, '{}.jpg'.format(curr_title)), dpi=300)
            pyplot.clf()
            pyplot.close()
    ...
    out = os.path.join('plots', act_type, 'main_text', 'lines')
    os.makedirs(out, exist_ok=True)
    for case, case_res in t_results.items():
        ...
        for analysis_type, an_res in case_res.items():
            chosen_color = tuple([min(255, rgb)/255 for rgb in line_colors[analysis_type]])
            line = list()
            for targ_idx, targ in enumerate(case_targets[case]):
                #y = max(an_res[targ])
