...
out_f = os.path.join('prediction_plots_shorter', 'main_text')
...
### correcting p values
        ...
        ax.set_ylim(bottom=-.09, top=0.45)
        ax.text(s='Language {}'.format(mapper[general_case]), x=0, y=.375, fontsize=25, fontweight='bold', fontstyle='italic', va='center', ha='center')
        pyplot.ylabel('Spearman correlation', fontsize=20, fontweight='bold')
        pyplot.xticks(ticks=range(len(xticks)), labels=xticks, fontsize=23, fontweight='bold')
...
pyplot.xticks(ticks=range(len(xticks)), labels=xticks, fontsize=23, fontweight='bold')
pyplot.yticks(fontsize=15)
ax.hlines(y=[_*0.1 for _ in range(-3, 5)], xmin=-.2, xmax=4.45, linestyle='--',color='gray', alpha=0.2)
ax.hlines(y=0., xmin=-.2, xmax=4.45, color='gray')
ax.set_ylim(bottom=-.09, top=0.45)
ax.legend(
          loc=9,
          fontsize=18,
