...
def check_preds(case, targ, pred_fam, pov, curr_weights):
    ...
    if pred_fam == 'activations':
        ...
        pred_len = 9
    assert len(curr_weights) in (pred_len, 2)
...
def check_confounds(case, targ, pred_fam, pov, curr_confs):
    ...
    print('confounds: {}'.format(curr_confs))
    #assert len(curr_confs) in [total_confs+_ for _ in range(-6, 6)]
    #print(curr_confs)
    ### checking everything is fine with time points
    poss_ts = ['T1', 'T2', 'T3']
    for p in curr_confs:
...
with open(os.path.join('pkls', 'main_text_ridge_predictions_{}.pkl'.format(args.choice)), 'rb') as i:
...
out_f = os.path.join('prediction_plots_{}'.format(args.choice), 'main_text')
    ...
    for key in [
        ...
        for pov in legend_mapper.keys():
            ...
            for x, scats in zip(pov_xs, scatters):
                ax.scatter([x+random.sample([corr*0.01 for corr in range(-10, 10)], k=1)[0] for _ in range(100)],
                            random.sample(scats.tolist(), k=100),
                           s=50,
                           color=colors[pov],
                           alpha=0.05,
                           )
...
if 't1t2' in args.choice:
    legend_mapper = {
        'activations T1' : 'Acute activity (unaffected hemisphere)',
        'activations T2' : 'Subacute activity (unaffected hemisphere)',
...
for case in cases:
    for targ in xtimes:
        ...
        for pred_fam in pred_fams:
            for pov in povs:
                if pov == 'T1' and 'conn' in pred_fam:
                    continue
                ...
                print(all_res[case][pov][targ].keys())
        ...
        for i, x, y, scats, details in zip(range(len(targ_ys)), targ_xs, targ_ys, targ_scats, targ_details):
            ...
            if curr_key == 'connectivity T1':
                continue
            ax.bar(x, y, width=0.15, alpha=0.4,  color=pred_colours[curr_key])
            ...
            ax.scatter([x+random.sample([corr*0.005 for corr in range(-10, 10)], k=1)[0] for _ in range(iter_null_hyp)],
                        random.sample(scats.tolist(), k=iter_null_hyp),
                       s=50,
...
for x, p, y, ci, pov, targ, raw_p, fam, case in zip(all_xs, corr_ps, all_ys, all_cis, curr_povs, curr_targs, all_ps, curr_fams, curr_cases):
    ...
    if p < 0.001:
        if y > 0.:
            ...
            p_y = -0.02
        ax.scatter(x-.025, p_y, s=200, marker='*', color='black', edgecolors='white', linewidth=1.)
        ax.scatter(x+.025, p_y, s=200, marker='*', color='black', edgecolors='white', linewidth=1.)
        ax.scatter(x, p_y+0.025, s=200, marker='*', color='black', edgecolors='white', linewidth=1.)
    elif p < 0.005:
        if y > 0.:
            p_y = 0.02
        else:
        ...
        if y > 0.:
            ...
            p_y = -0.02
        ax.scatter(x, p_y, s=200, marker='*', color='black', edgecolors='white', linewidth=1.)
    elif p < 0.11:
        continue
        if y > 0.:
            p_y = 0.02
...
pyplot.yticks(fontsize=15)
ax.hlines(y=[_*0.1 for _ in range(-1, 6)], xmin=-.2, xmax=5.45, linestyle='--',color='gray', alpha=0.2)
ax.hlines(y=0., xmin=-.2, xmax=5.45, color='gray')
ax.set_ylim(bottom=-.12, top=0.59)
ax.legend(
          loc=2,
          fontsize=18,
          ncols=6,
...
if 't1t2' in args.choice:
    possibilities = [
                 ('activations', 'T1', 'abilities', 'T1'),
                 ('activations', 'T2', 'abilities', 'T3'),
                 ('activations', 'T3', 'abilities', 'T2'),
                 ('activations', 'T3', 'abilities', 'T3'),
                 ('activations', 'T1', 'improvements', 'T2-T1'),
                 ('activations', 'T2', 'improvements', 'T2-T1'),
                 ('connectivity', 'T2', 'improvements', 'T2-T1'),
                 ('activations', 'T1', 'improvements', 'T3-T1'),
                 ('activations', 'T2', 'improvements', 'T3-T1'),
            ...
            for targ in all_res[case][pov].keys():
                for fam in pred_fams:
                    ...
                    if p_check not in possibilities:
                        continue
                    if 'conn' in fam and 'T2' in pov and 't1t2' in args.choice:
                        continue
                    if general_case == 'correlations':
                        all_ks = [k for k in all_res[case][pov][targ][general_case].keys() if fam in k and ' and ' in k]
                        assert len(all_ks) in [len(orders[fam]), 2, 1]
                        x_names = list()
                        for k in orders[fam]:
                            for a_k in all_ks:
                                if k in a_k:
                                    x_names.append(a_k)
                        assert len(x_names) in [len(orders[fam]), 2,1]
        ...
        for pov in all_res[case].keys():
            for targ in all_res[case][pov].keys():
                for fam in pred_fams:
                    ...
                    if general_case == 'correlations':
                        all_ks = [k for k in all_res[case][pov][targ][general_case].keys() if fam in k and ' and ' in k]
                        assert len(all_ks) in [len(orders[fam]), 2]
                        x_names = list()
                        for k in orders[fam]:
                            for a_k in all_ks:
                                if k in a_k:
                                    x_names.append(a_k)
                        assert len(x_names) in [len(orders[fam]), 2]
