...
with open(os.path.join('dataset', 'sex_41.tsv')) as i:
    for l_i, l in enumerate(i):
        ...
        raw_data['sex'][l_i-1] = -.5 if line[1] == 'm' else .5
...
labels['T1_day_after_onset'] = ['T1_day_after_onset']
labels['T2_day_after_onset'] = ['T2_day_after_onset']
labels['T3_day_after_onset'] = ['T3_day_after_onset']
...
for name, targets in [
    ...
    for pr in predictors:
        for t in targets:
            if pr not in predictors[:2] and int(pr[-1]) >= int(t[-1]):
                continue
            key, dict_t = read_dict_keys(name, t)
            ### updating collector dict
            collector = check_collector(collector, key, dict_t)
            ### the other predictor, from the present
            other_predictors = [l for l in predictors if (l in predictors[:2] and l!=pr) or (l not in predictors[:2] and int(l[-1])<=int(t[-1]) and l!=pr)]
            assert len(other_predictors) in range(2, 10)
            ...
            other_confounds_names = [k for l in predictors for k in labels[l] if (l in predictors[:2] and l in other_predictors and k!=t) or (l not in predictors[:2] and int(l[-1])<=int(t[-1]) and l in other_predictors and k!=t)]
            confounds_names = orig_confounds_names+other_confounds_names

            ### weights from present & family
            weights_names = [k for l in predictors for k in labels[l] if (l in predictors[:2] and l==pr) or (l not in predictors[:2] and int(l[-1])<=int(t[-1]) and l==pr)]
...
with open(os.path.join(out_f, 'si_{}_predictions_t1right-6733.pkl'.format(pred_model)), 'wb') as o:
