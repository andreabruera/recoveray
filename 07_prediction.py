    ...
    for t in targets:
        ...
        for point_of_view in abilities:
            ...
            if dict_t not in collector[key][point_of_view].keys():
                collector[key][point_of_view][dict_t] = dict()
            ### full case
            confounds_names = [k for l in confounds for k in labels[l]]
            weights_names = [k for l in predictors for k in labels[l] if l=='age' or int(l[-1]) in [
                                                                                                    int(point_of_view[-1])
                                                                                                    ]]
            ...
            for null in tqdm(range(iter_null_hyp)):
                ...
                iters.append(t_avg)
            collector[key][point_of_view][dict_t]['confounds'] = confounds_names
            collector[key][point_of_view][dict_t]['predictors'] = weights_names
            collector[key][point_of_view][dict_t]['correlations'] = results_container
            ### p value
            ...
            ### weights
            collector[key][point_of_view][dict_t]['weights'] = weights_container
            collector[key][point_of_view][dict_t]['weights_names'] = weights_names
    ...
    for pr in predictors:
        for t in targets:
            ...
            for point_of_view in abilities:
                ...
                other_confounds_names = [k for l in predictors for k in labels[l] if (l=='age' or int(l[-1]) in [
                                                                                                         int(point_of_view[-1]),
                                                                                                         ]) and l==other_pr]
                ...
                collector[key][point_of_view][dict_t]['confounds'] = confounds_names
                collector[key][point_of_view][dict_t]['predictors'] = weights_names
                collector[key][point_of_view][dict_t]['only {}'.format(pr)] = results_container
                ### p value
                p_container = numpy.average(results_container, axis=1)
                assert p_container.shape == (iter_null_hyp, )
                p = (sum([1 for _ in p_container if _<0])+1)/(iter_null_hyp+1)
                ...
                ### aggregate result
                t_avg = numpy.nanmean(p_container)
                print(['{} only {}, from {}'.format(dict_t, pr, point_of_view), 'avg: {}'.format(round(t_avg, 3)), 'p: {}'.format(round(p, 3))])
                print('\n')
        ...
        for t in targets:
            ...
            for point_of_view in abilities:
                ...
                for ind_pred in labels[pr]:
                    ...
                    collector[key][point_of_view][dict_t]['confounds'] = confounds_names
                    collector[key][point_of_view][dict_t]['predictors'] = weights_names
                    collector[key][point_of_view][dict_t]['only {} and {}'.format(pr, ind_pred)] = results_container
                    ### p value
                    p_container = numpy.average(results_container, axis=1)
                    assert p_container.shape == (iter_null_hyp, )
                    p = (sum([1 for _ in p_container if _<0])+1)/(iter_null_hyp+1)
                    ...
                    ### aggregate result
                    t_avg = numpy.nanmean(p_container)
                    print(['{} only {} w/o {}, from {}'.format(dict_t, pr, ind_pred, point_of_view), 'avg: {}'.format(round(t_avg, 3)), 'p: {}'.format(round(p, 3))])
                    print('\n')
with open('ridge_predictions.pkl', 'wb') as o:
