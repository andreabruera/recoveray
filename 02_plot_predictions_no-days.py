...
def check_confounds(case, targ, pred_fam, pov, curr_confs):
    ...
    #print(len(curr_confs))
    assert len(curr_confs) in [total_confs+_ for _ in range(4)]
    #print(curr_confs)
    ### checking everything is fine with time points
    poss_ts = ['T1', 'T2', 'T3']
    for p in curr_confs:
...
with open(os.path.join('pkls', 'main_text_ridge_predictions_no-days.pkl'), 'rb') as i:
...
out_f = os.path.join('prediction_plots_no-days', 'main_text')
...
for case in cases:
    for targ in xtimes:
        ...
        for i, x, y, scats, details in zip(range(len(targ_ys)), targ_xs, targ_ys, targ_scats, targ_details):
            ...
            ax.scatter([x+random.sample([corr*0.005 for corr in range(-10, 10)], k=1)[0] for _ in range(100)],
                        random.sample(scats.tolist(), k=100),
                       s=50,
...
ax.set_ylim(bottom=-.15, top=0.45)
ax.legend(
          loc=2,
          fontsize=18,
          ncols=6,
